...
class EventGenerator
{
    ...
    {
        $eventName = $this->askClassName('Event name?');
        $exitCode = $this->preview->previewAndRun('make:event', ['name' => $eventName]);
        ...
        if ($exitCode === SymfonyCommand::SUCCESS) {
            $this->command->info('Event created successfully');
            $this->logAction(sprintf('User created event: %s', $eventName));
        } else {
            $this->command->error('Event generation failed');
        }
