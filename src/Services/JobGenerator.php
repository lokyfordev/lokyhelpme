...
class JobGenerator
{
    ...
    {
        $jobName = $this->askClassName('Job name?');
        $exitCode = $this->preview->previewAndRun('make:job', ['name' => $jobName]);
        ...
        if ($exitCode === SymfonyCommand::SUCCESS) {
            $this->command->info('Job created successfully.');
            $this->logAction(sprintf('User created job: %s', $jobName));
        } else {
            $this->command->error('Job generation failed.');
        }
